...
- **完成内容**: Header 得分区域 flex-wrap 防止窄屏挤压；Trophy/Award 文字在 lg 以下隐藏仅显示图标；标题响应式 text-base md:text-lg；ToggleGroup 移动端 overflow-x-auto + flex-col 布局；PracticeCard 输入框弹性宽度 min-w/max-w；音频按钮响应式尺寸 h-8 md:h-10；新增 4 个响应式布局测试
- **当前状态**: **连续未被选中计数清零**（作为 epic-004 提议者）
- **观察**: epic-004 是 PM-UX 在 epic-003 全部完成后提出的新 Epic，直接响应用户完整体验评测反馈（dir-user-review-001）。iter-001 作为 P0 移动端适配，零新增依赖、零新增组件、零构建体积增长，纯 Tailwind 工具类调整。这是典型的「微调型」迭代——风险极低，验证成本小，但用户价值极高（移动端从不可用变为可用）。后续 iter-002~005 将继续按 P1~P3 优先级推进体验优化

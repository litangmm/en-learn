...
- **当前状态**: **连续未被选中计数保持清零**（epic-083 支持者）
- **观察**: iter-003 体现了 UX「学习动机维持」理念的「效率感知」维度——PersonalDictionaryStatsPanel 的 6 种效率指标（词数/掌握度/掌握等级分布/平均间隔/最长未复习/高错误率词）让用户量化感知自己的学习效率。掌握度颜色编码（green/amber/red）和掌握等级分布进度条提供了清晰的视觉反馈，符合「清晰目标+即时进度」原则。**epic-083 完成度 3/4，剩余 iter-004（遗忘曲线可视化），继续支持**
...
- **完成内容**: BadgeExportPanel 组件（网格预览+下载PNG）+ useBadgeExport Hook（html2canvas 图片导出）+ BadgePanel 导出按钮集成；复用 html2canvas，零新增依赖
- **当前状态**: **连续未被选中计数保持清零**（epic-029 支持者）
- **观察**: iter-004 的徽章墙导出是 UX「学习心流」理念的「成果可视化」维度延伸——用户可将个人成就（解锁的徽章）导出为图片分享，强化成就感和社交传播动机。BadgeExportPanel 的网格预览让用户在导出前预览效果，符合 UX 的「所见即所得」原则。**epic-029 仅剩 iter-005（数据可视化）继续支持，epic-029 即将全部收官**

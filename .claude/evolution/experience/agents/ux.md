...
- **完成内容**: getModeHint 模式提示文案（4 种模式）；renderWrongAnswerFeedback 三区块反馈（你的答案/正确答案/解析）；CheckCircle2/Circle 图标增强；连词成句进度文字
- **当前状态**: **连续未被选中计数清零**（以支持者身份参与 epic-004）
- **观察**: iter-002 的三区块结构化反馈设计完全符合教育心理学中的「建构主义反馈」原则——不仅告诉用户"错了"，还提供正确答案和解释，帮助用户主动建构正确认知。CheckCircle2/Circle 图标的加入使选择题的选中状态在视觉上更清晰可辨，与 UX 的「即时反馈」理念高度一致。模式提示文案让用户在上场前就理解交互规则，降低了认知负荷

...
describe('App mode hints', () => {
  beforeEach(() => {
    cleanup();
    vi.clearAllMocks();
  });

  it('shows fill-in-blanks hint by default', () => {
    render(<App />);
    expect(
      screen.getByText('听音频后，在输入框中填入缺失的单词，按 Enter 键快速提交'),
    ).toBeInTheDocument();
  });

  it('shows dictation hint when switching to dictation mode', () => {
    render(<App />);
  ...
  it('shows multiple-choice hint when switching to multiple-choice mode', () => {
    render(<App />);
    const mcButton = screen.getByText('选择题模式');
    fireEvent.click(mcButton);

    expect(
      screen.getByText('选择最合适的答案后，点击提交答案按钮'),
    ).toBeInTheDocument();
  });

  it('shows sentence-reorder hint when switching to sentence-reorder mode', () => {
    render(<App />);
    const reorderButton = screen.getByText('连词成句');
    fireEvent.click(reorderButton);

    expect(
      screen.getByText('点击单词组成正确句子，再次点击已选单词可撤回'),
    ).toBeInTheDocument();
  });

  it('hides hint in focus mode', () => {
    render(<App />);
    ...
    expect(
      screen.queryByText('听音频后，在输入框中填入缺失的单词，按 Enter 键快速提交'),
